...
def build_dashboard(signals_df, portfolio_evaluation_df):
   ...
   # Create hvplot visualizations
    ...
    width=700,
    height=400,
    title='TSLA, , Long Strategy, Crossover SMA 30 over SMA 120')
...
def generate_signals(data_df):
    ...
    signals_df["SMA30"] = signals_df["Close"].rolling(window=short_window).mean()
    signals_df["SMA120"] = signals_df["Close"].rolling(window=long_window).mean()
    ...
    signals_df["Signal"][short_window:] = np.where(
        signals_df["SMA30"][short_window:] > signals_df["SMA120"][short_window:],
        1.0,
        0.0,
    )
